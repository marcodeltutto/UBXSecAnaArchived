...
//root [1] TChain * c = new TChain("pandoraCosmicStoppingMu/tree");
//root [2] c->Add("../files/mcc8.3_stopmu_test/ubxsec_output_data_extbnb_mcc8.3_stopmu_test14.root");
//root [3] AnaTree t(c)
//root [4] t.Loop()
...
void AnaTree::Loop()
{
  ...
  TH1D * delta_ll_cosmic_stopmu = new TH1D("delta_ll_cosmic_stopmu", "delta_ll_cosmic_stopmu", 1000, -50, 50);
  TH1D * delta_ll_cosmic_nostopmu = new TH1D("delta_ll_cosmic_nostopmu", "delta_ll_cosmic_nostopmu", 1000, -50, 50);
  TH1D * delta_ll_neutrino = new TH1D("delta_ll_neutrino", "delta_ll_neutrino", 1000, -50, 50);
  TH1D * delta_ll_all = new TH1D("delta_ll_all", "delta_ll_all", 1000, -50, 50);
  ...
  for (double cut_value = 5; cut_value >= -8; cut_value -=2) {
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if(first) delta_ll_all->Fill(delta_ll);
      ...
      if (origin == 1 && origin_extra == 0) {
        total_B_stopmu++;
        if(first) delta_ll_cosmic_stopmu->Fill(delta_ll);
      }
      if (origin == 1 && origin_extra != 0) {
        if(first) delta_ll_cosmic_nostopmu->Fill(delta_ll);
      }
      if (origin == 0) {
        total_S ++;
        if(first) delta_ll_neutrino->Fill(delta_ll);
      }
  ...
  delta_ll_cosmic_stopmu->Draw("same");
  delta_ll_cosmic_nostopmu->Draw("same");
  delta_ll_all->SetLineColor(kBlack);
  delta_ll_all->SetLineWidth(3);
  delta_ll_all->Draw("same");
  
  TFile *file_out = new TFile("mcs_ana_output.root","RECREATE");
  if ( file_out->IsOpen() )
    std::cout << "File opened successfully" << std::endl;
  
  file_out->cd();
  delta_ll_all->Write();
  delta_ll_cosmic_nostopmu->Write();
  delta_ll_cosmic_stopmu->Write();
  delta_ll_neutrino->Write();
